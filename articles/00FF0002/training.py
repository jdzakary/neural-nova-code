...
from enviornment import MegaTicTacToe, MegaTicTacToeSingle
...
def single_env_creator(env_config: dict):
    return MegaTicTacToeSingle(env_config)
...
def main():
    ...
    max_time_total = 60 * 60 * 1
    grace_period_iter = 20
    ...
    register_env('tic-tac-toe', single_env_creator)
    config = (
        PPOConfig()
        .api_stack(
    ...
    config_checkpoint = train.CheckpointConfig(
        ...
        checkpoint_score_attribute='env_runners/Win',
    )

    # Create Tuner Config
    config_tuner = tune.TuneConfig(
        metric='env_runners/Win',
        mode='max',
